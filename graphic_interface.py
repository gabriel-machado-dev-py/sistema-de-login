...
class BackEnd():
    ...
    def cadastrar_usario(self) -> None:
        ...
        hashed_password = hashlib.sha256(self.password_entry.encode()).hexdigest()
        hashed_confirm_password = hashlib.sha256(self.confir_password_entry.encode()).hexdigest()

        self.cursor.execute("""
                            INSERT INTO Usuarios(user, email, password, confirm_password)
                            VALUES (?, ?, ?, ?)
                            """, (self.username_entry, self.email_entry, hashed_password, hashed_confirm_password))
        try:
    ...
    def logar_usuario(self) -> None:
        ...
        self.cursor.execute("""
                            ...
                            """, (self.username_entry, hashed_password))
